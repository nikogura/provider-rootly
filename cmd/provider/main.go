...
import (
	...
	apisCluster "github.com/nikogura/provider-rootly/apis/cluster"
	apisNamespaced "github.com/nikogura/provider-rootly/apis/namespaced"
	"github.com/nikogura/provider-rootly/config"
	"github.com/nikogura/provider-rootly/internal/clients"
	controllerCluster "github.com/nikogura/provider-rootly/internal/controller/cluster"
	controllerNamespaced "github.com/nikogura/provider-rootly/internal/controller/namespaced"
	"github.com/nikogura/provider-rootly/internal/features"
	"github.com/nikogura/provider-rootly/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Rootly").DefaultEnvars()
		...
		certsDir = app.Flag("certs-dir", "The directory that contains the server key and certificate.").Default(tlsServerCertDir).Envar(certsDirEnvVar).PreAction(func(_ *kingpin.ParseContext) (err error) {
			certsDirSet = true
			return err
		}).String()
	)
	...
	log := logging.NewLogrLogger(zl.WithName("provider-rootly"))
	if *debug {
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(scheme), "Cannot add cluster-scoped Rootly APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(scheme), "Cannot add namespace-scoped Rootly APIs to scheme")
	kingpin.FatalIfError(apiextensionsv1.AddToScheme(scheme), "Cannot add CustomResourceDefinition to scheme")

	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		Scheme:           scheme,
		LeaderElection:   *leaderElection,
		LeaderElectionID: "crossplane-leader-election-provider-rootly",
		Cache: cache.Options{
		...
		LeaseDuration:              func() (d *time.Duration) { v := 60 * time.Second; d = &v; return d }(),
		RenewDeadline:              func() (d *time.Duration) { v := 50 * time.Second; d = &v; return d }(),
	})
	kingpin.FatalIfError(err, "Cannot create controller manager")
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped Rootly APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced Rootly APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped Rootly controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced Rootly controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped Rootly controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced Rootly controllers")
	}

	kingpin.FatalIfError(mgr.Start(ctrl.SetupSignalHandler()), "Cannot start controller manager")
}

func canWatchCRD(ctx context.Context, mgr manager.Manager) (allowed bool, err error) {
	err = authv1.AddToScheme(mgr.GetScheme())
	if err != nil {
		return allowed, err
	}
	...
	for _, verb := range verbs {
		...
		err = mgr.GetClient().Create(ctx, sar)
		if err != nil {
			err = errors.Wrapf(err, "unable to perform RBAC check for verb %s on CustomResourceDefinitions", verbs)
			return allowed, err
		}
