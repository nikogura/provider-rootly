...
PROJECT_NAME ?= provider-rootly
PROJECT_REPO ?= github.com/nikogura/$(PROJECT_NAME)
...
export TERRAFORM_PROVIDER_SOURCE ?= rootlyhq/rootly
export TERRAFORM_PROVIDER_REPO ?= https://github.com/rootlyhq/terraform-provider-rootly
...
XPKG_REG_ORGS ?= ghcr.io/nikogura
# NOTE(hasheddan): skip promoting on xpkg.crossplane.io as channel tags are
# inferred.
XPKG_REG_ORGS_NO_PROMOTE ?= ghcr.io/nikogura
XPKGS = $(PROJECT_NAME)
...
xpkg.build.provider-rootly: do.build.images
...
# namedreturns
#
# The namedreturns linter (github.com/nikogura/namedreturns) is mandatory and runs
# BEFORE golangci-lint, fail-fast: it is hooked as lint.init, which the build
# system's `lint` target runs before lint.run (golangci-lint). Generated code is
# exempt -- apis/... and internal/controller/... are Upjet output. Everything
# hand-maintained is checked, test files included (-test=true, the default).

NAMEDRETURNS := $(TOOLS_HOST_DIR)/namedreturns
NAMEDRETURNS_PACKAGES := ./cmd/... ./config/... ./internal/clients ./internal/features ./internal/version

$(NAMEDRETURNS):
	@$(INFO) installing namedreturns
	@mkdir -p $(TOOLS_HOST_DIR) || $(FAIL)
	@GOBIN=$(TOOLS_HOST_DIR) GOFLAGS= $(GO) install github.com/nikogura/namedreturns@latest || $(FAIL)
	@$(OK) installing namedreturns

namedreturns.lint: $(NAMEDRETURNS)
	@$(INFO) namedreturns
	@for pkg in $$($(GO) list $(NAMEDRETURNS_PACKAGES) | grep -v '/internal/controller'); do \
		$(NAMEDRETURNS) -test=true $$pkg || exit 1; \
	done || $(FAIL)
	@$(OK) namedreturns

lint.init: namedreturns.lint

.PHONY: namedreturns.lint
